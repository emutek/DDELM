...
parser.add_argument(
  "--problem", "-p", type=str, default='grf_16_0b_1f',
  choices=[
    'grf_16_0b_1f', 'grf_32_0b_1f',
    ],
...
parser.add_argument("--ini_l", type=float, default=None, help="initialization length scale")
...
d_act = vmap(_d_act)
dd_act = vmap(_dd_act)
class PoissonVarYKDD(base_class.YKDD):
  def __init__(self, **kwargs):
    super().__init__(**kwargs)
    self.act = act
    if 'target_exact' in kwargs:
      self.target_exact = kwargs['target_exact']
      self.target_exact_grad = kwargs['target_exact_grad']

  @base_class.nonkwargs_to_kwargs
  def define_problem_params(
    self, *,
    coeff: Callable[[Tensor], Tensor],
    target_rhs: Callable[[Tensor], Tensor],
    target_bou: Callable[[Tensor], Tensor]
  ) -> None:
    self.coeff = coeff
    self.target_rhs = target_rhs
    self.target_bou = target_bou

  def dx_coeff(self, x: Tensor) -> Tensor:
    return jacrev(self.coeff)(x)

  def Lnet_x_single(self, x: Tensor, theta) -> Tensor:
    w, b = theta['w'], theta['b']
    return -(self.coeff(x) * dd_act(x @ w + b) * (w * w).sum(dim=0) + d_act(x @ w + b) * torch.einsum('dM, d->M', w, self.dx_coeff(x)))
  
  def fluxnet_x_single(self, x: Tensor, theta, flux_info: Tensor) -> Tensor:
    w, b = theta['w'], theta['b']
    return self.coeff(x) * d_act(x @ w + b) * torch.einsum('dM, d->M', w, flux_info)
...
def get_problem(problem: str):
  ...
  if problem == 'grf_16_0b_1f':
    a, w, b = awb_from_path('grf_coeffs/poisson/a16_M256_seed42')
    coeff, _, _ = grf_coeffs(a, w, b)
    target_rhs = lambda x: ones(x.shape[:-1])
    target_bou = lambda x: zeros(x.shape[:-1])
    u_fem = np.load(fenicsx_path / 'u0_plot_var_grf_16_0b_1f_32_16_1025.npy').reshape(1025, 1025)
    u_fem_grad = np.load(fenicsx_path / 'u0_plot_var_grf_16_0b_1f_32_16_1025_grad.npy').reshape(1025, 1025, 2)
    target_exact, target_exact_grad = get_fem_sols(u_fem, u_fem_grad, grad_idx=1)

  elif problem == 'grf_32_0b_1f':
    a, w, b = awb_from_path('grf_coeffs/poisson/a32_M256_seed43')
    coeff, _, _ = grf_coeffs(a, w, b)
    target_rhs = lambda x: ones(x.shape[:-1])
    target_bou = lambda x: zeros(x.shape[:-1])
    u_fem = np.load(fenicsx_path / 'u0_plot_var_grf_32_0b_1f_32_16_1025.npy').reshape(1025, 1025)
    u_fem_grad = np.load(fenicsx_path / 'u0_plot_var_grf_32_0b_1f_32_16_1025_grad.npy').reshape(1025, 1025, 2)
    target_exact, target_exact_grad = get_fem_sols(u_fem, u_fem_grad, grad_idx=1)

  return coeff, target_rhs, target_bou, target_exact, target_exact_grad
  
if __name__ == '__main__':
  t1 = time.time()
  coeff, target_rhs, target_bou, target_exact, target_exact_grad = get_problem(args.problem)
  ...
  ykdd = PoissonVarYKDD(
    ...
    target_exact_grad = target_exact_grad
  )
  ...
  l = args.ini_l if args.ini_l is not None else (M * size / 64) ** .5
  ...
  error_grid = grid if args.error_n is None else ykdd.get_grid(n=args.error_n)
  ...
  if args.save_img_paper:
    ...
    if rank == 0:
      ...
      uses_fem_ref = args.problem in {'grf_16_0b_1f', 'grf_32_0b_1f'}
      if uses_fem_ref:
        y_ref = np.load(f'./fem_sols_grid2/u0_plot_var_{args.problem}_32_16_{N}_{plot_n}.npy')
      ...
      savefig = lambda title: plt.savefig(f'figures/poivar_{args.problem}_{N}_{args.plot_n}_{title}.{save_format}', format=save_format, bbox_inches='tight')
